...
class BaseModel():
            ...
            if self.epoch % self.opt['train']['val_epoch'] == 0:
                ...
                if self.val_loader is None:
                    ...
                    for key, value in val_log.items():
                        self.logger.info('{:5s}: {}\t'.format(str(key), value))
                self.logger.info("\n--------------------Validation End----------------------")
        self.logger.info('Number of Epochs has reached the limit, The End.')
    ...
    def print_network(self):
        ...
        if self.opt['global_rank'] != 0:
            return
        if isinstance(network, nn.DataParllel) or isinstance(network, nn.parallel.DistributedDataParallel):
    ...
    def save_everything(self):
        raise NotImplementedError("You must specify how to save your networks, optimizer and schedulers.")